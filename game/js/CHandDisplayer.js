function CHandDisplayer (iOffsetX,iOffsetY,iX,iY,iPlayerID,oParentContainer,szPlayerName,iSide){
    ...
    this.setPosition = function(iNewOffsetX,iNewOffsetY,iNewX,iNewY,iSide){
        ...
        switch(_iSide){
            case BOTTOM:{
                    oInfoPos = {
                        x: UI_SIZES_AND_POSITIONS[currentUI].player_info.bottom(_oContainer).x, 
                        y: UI_SIZES_AND_POSITIONS[currentUI].player_info.bottom(_oContainer).y
                    };
                    break;
            }
            case TOP:{
                    _iOffsetX = -iNewOffsetX;
                    oInfoPos = {
                        x: UI_SIZES_AND_POSITIONS[currentUI].player_info.top(_oContainer).x, 
                        y: UI_SIZES_AND_POSITIONS[currentUI].player_info.top(_oContainer).y
                    };
                    break;
            }
            case LEFT:{
                    oInfoPos = {
                        x: UI_SIZES_AND_POSITIONS[currentUI].player_info.left(_oContainer).x, 
                        y: UI_SIZES_AND_POSITIONS[currentUI].player_info.left(_oContainer).y
                    };
            ...
            case RIGHT:{
                    _iOffsetY = -iNewOffsetY;
                    oInfoPos = {
                        x: UI_SIZES_AND_POSITIONS[currentUI].player_info.right(_oContainer).x, 
                        y: UI_SIZES_AND_POSITIONS[currentUI].player_info.right(_oContainer).y
                    };
                    _oSummaryScore.setPosition(_oContainer.x-20,_oContainer.y,_iSide);
                    break;
            }
    ...
    this.getGlobalPosition = function(){
        ...
        for (var i=0;i<_aCards.length;i++){
            if (_iOffsetX!==0){
                oCardsInfoX += UI_SIZES_AND_POSITIONS[currentUI].player_info.globalPos().x;
            }else{
                oCardsInfoY += UI_SIZES_AND_POSITIONS[currentUI].player_info.globalPos().y;
            }
        }
    ...
    this.centerContainer = function(){
        ...
        var iOffsetX = UI_SIZES_AND_POSITIONS[currentUI].center_container(oHandInfo, iOffsetY).x;
        if(_iSide === TOP){
            iOffsetX = -UI_SIZES_AND_POSITIONS[currentUI].center_container(oHandInfo, iOffsetY).x;
        }
        var iOffsetY = UI_SIZES_AND_POSITIONS[currentUI].center_container(oHandInfo, iOffsetY).y;
        
        if(_iSide === RIGHT){
            iOffsetY = -UI_SIZES_AND_POSITIONS[currentUI].center_container(oHandInfo, iOffsetY).y;
        }
        
        if (_iOffsetX !== 0) {
            createjs.Tween.get(_oContainer).to({x: CANVAS_WIDTH / 2 + iOffsetX }, 300, createjs.Ease.linear);
        } else {
            createjs.Tween.get(_oContainer).to({y: UI_SIZES_AND_POSITIONS[currentUI].center_container(oHandInfo, iOffsetY).tweenY}, 300, createjs.Ease.linear);
        }
    };
        ...
        if (_aCards.length===1){
            ...
            if (_iOffsetX!==0){
                if (_oContainer.y<CANVAS_HEIGHT/2){
                    ...
                    _oCloudUno.x = UI_SIZES_AND_POSITIONS[currentUI].top_cloud().x;
                    _oCloudUno.y = UI_SIZES_AND_POSITIONS[currentUI].top_cloud().y;
                }else{
                   _oCloudUno.gotoAndStop("cloud1");
                   _oCloudUno.x = UI_SIZES_AND_POSITIONS[currentUI].bottom_cloud().x;
                   _oCloudUno.y = UI_SIZES_AND_POSITIONS[currentUI].bottom_cloud().y;
                }
            }else{
                if (_oContainer.x<CANVAS_WIDTH/2){
                    _oCloudUno.gotoAndStop("cloud1");
                    _oCloudUno.x = UI_SIZES_AND_POSITIONS[currentUI].left_cloud().x;
                    _oCloudUno.y = UI_SIZES_AND_POSITIONS[currentUI].left_cloud().y;
                }else{
                    _oCloudUno.gotoAndStop("cloud2");
                    _oCloudUno.x = UI_SIZES_AND_POSITIONS[currentUI].right_cloud().x;
                    _oCloudUno.y = UI_SIZES_AND_POSITIONS[currentUI].right_cloud().y;
                }
            }
            s_oStage.addChild(_oCloudUno);
            new createjs.Tween.get(_oCloudUno).to({alpha: 1, scaleX:  UI_SIZES_AND_POSITIONS[currentUI].right_cloud().scale, scaleY: UI_SIZES_AND_POSITIONS[currentUI].right_cloud().scale},300, createjs.Ease.bounceOut).wait(1500).to({ scaleX: 0.1, scaleY: 0.1},300, createjs.Ease.cubicIn).to({alpha: 0},20).call(function(){s_oStage.removeChild(_oCloudUno);});
        }
    };
