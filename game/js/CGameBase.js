...
CGameBase.prototype._init = function(){
    ...
    oBg.scaleX = oBg.image.width < CANVAS_WIDTH ? 1.2 : UI_SIZES_AND_POSITIONS[currentUI].game_oBg(oBg).scale;
    oBg.scaleY = oBg.image.height < CANVAS_HEIGHT ? 1.2 : UI_SIZES_AND_POSITIONS[currentUI].game_oBg(oBg).scale;
    oBg.x = oBg.image.width < CANVAS_WIDTH ? UI_SIZES_AND_POSITIONS[currentUI].game_oBg(oBg).x - 100 : UI_SIZES_AND_POSITIONS[currentUI].game_oBg(oBg).x
    oBg.y = oBg.image.height < CANVAS_HEIGHT ? UI_SIZES_AND_POSITIONS[currentUI].game_oBg(oBg).y - 100 : UI_SIZES_AND_POSITIONS[currentUI].game_oBg(oBg).y
    s_oStage.addChild(oBg); 
    if(currentUI=="MOBILE_DEVICE"){
        // var oBgMask = createBitmap(s_oSpriteLibrary.getSprite('bg_mask'));
        // oBgMask.scaleX = oBgMask.image.width < CANVAS_WIDTH ? 1.2 : 1;
        // oBgMask.scaleY = oBgMask.image.height < CANVAS_HEIGHT ? 1.2 : 1;
        // oBgMask.x = oBgMask.image.width < CANVAS_WIDTH ? UI_SIZES_AND_POSITIONS[currentUI].game_oBgMask(oBgMask).x - 100 : UI_SIZES_AND_POSITIONS[currentUI].game_oBgMask(oBgMask).x;
        // oBgMask.y = oBgMask.image.height < CANVAS_HEIGHT ? UI_SIZES_AND_POSITIONS[currentUI].game_oBgMask(oBgMask).y - 100 : UI_SIZES_AND_POSITIONS[currentUI].game_oBgMask(oBgMask).y;
        // s_oStage.addChild(oBgMask); 
    } 
    if(currentUI=="MOBILE_DEVICE" || currentUI==="IPAD_DEVICE_3X4" || currentUI==="IPAD_DEVICE"){
        var oBgLogo = createBitmap(s_oSpriteLibrary.getSprite('bg_logo'));
        oBgLogo.scaleX = UI_SIZES_AND_POSITIONS[currentUI].game_oBgLogo(oBg).scale
        oBgLogo.scaleY = UI_SIZES_AND_POSITIONS[currentUI].game_oBgLogo(oBg).scale
        oBgLogo.x = UI_SIZES_AND_POSITIONS[currentUI].game_oBgLogo(oBgLogo).x;
        oBgLogo.y = UI_SIZES_AND_POSITIONS[currentUI].game_oBgLogo(oBgLogo).y;
        s_oStage.addChild(oBgLogo); 
        var oBgCenterBox = createBitmap(s_oSpriteLibrary.getSprite('bg_center_box'));
        oBgCenterBox.scaleX = UI_SIZES_AND_POSITIONS[currentUI].game_bg_center_box(oBg).scale
        oBgCenterBox.scaleY = UI_SIZES_AND_POSITIONS[currentUI].game_bg_center_box(oBg).scale
        oBgCenterBox.x = UI_SIZES_AND_POSITIONS[currentUI].game_bg_center_box(oBgCenterBox).x;
        oBgCenterBox.y = UI_SIZES_AND_POSITIONS[currentUI].game_bg_center_box(oBgCenterBox).y;
        s_oStage.addChild(oBgCenterBox);
    }
    ...
    for (var i=0;i<NUM_PLAYERS;i++){
        var iOffsetX =0;
        var iOffsetY =0;
        if (UI_SIZES_AND_POSITIONS[currentUI].handPositions().pos[i].x===CANVAS_WIDTH/2){
            iOffsetX = UI_SIZES_AND_POSITIONS[currentUI].player_info.offset().x;            
        }else{
            iOffsetY = UI_SIZES_AND_POSITIONS[currentUI].player_info.offset().y;
        }
        this._aPlayersHand[i] = new CHandDisplayer(iOffsetX,iOffsetY,
            UI_SIZES_AND_POSITIONS[currentUI].handPositions().pos[i].x,
            UI_SIZES_AND_POSITIONS[currentUI].handPositions().pos[i].y,
            i,
            this._oHandsContainer,
            aTextPlayers[i],
            UI_SIZES_AND_POSITIONS[currentUI].handPositions().pos[i].side);
    }
    ...
    this._oDeck = new CDeckDisplayer(UI_SIZES_AND_POSITIONS[currentUI]._oDeck().x,UI_SIZES_AND_POSITIONS[currentUI]._oDeck().y,this._oDecksContainer, true);
    this._oUsedCards = new CDeckDisplayer(UI_SIZES_AND_POSITIONS[currentUI]._oUsedCards().x,UI_SIZES_AND_POSITIONS[currentUI]._oUsedCards().y,this._oDecksContainer, false);
    ...
    globalThis.gameManager.addRewardHandler("FOUR_COLORS_SWAP", () => {
        var oSelectColorPanel = new CSelectColorPanel(EFFECT_SELECT_COLOR);
            oSelectColorPanel.addEventListener(ON_COLOR_SELECTED, function(iColor){
                ...
                this._onVideoCompletedWatched(oData);
...
CGameBase.prototype.reset = function(){
    ...
    this._oInterface._noOfTimesSwapped = 0;
...
CGameBase.prototype.onCardDealed = function(oCard){
    ...
    if (!this.matchCanStart()){
        ...
        oCard.moveFirstLastCard(CARD_WIDTH /2,0,600);
    }
};
...
CGameBase.prototype.onFirstLastCardDealed = function (oCard){
    ...
    if(currentUI==="MOBILE_DEVICE"){
        ...
        _oDeck_container_move_x = CANVAS_WIDTH/2-CARD_WIDTH/2,
        _oUsedCards_container_move_x = CANVAS_WIDTH/2+CARD_WIDTH/2
    }
    ...
    this._oDeck.moveContainer(_oDeck_container_move_x,CANVAS_HEIGHT/2,400);
...
CGameBase.prototype.cpuPlayCard = function(oParent){
    ...
    this._oDeck.setChildDepth(currentUI ==="DESKTOP_DEVICE" ? 2 : 4);
    this._oUsedCards.setChildDepth(currentUI ==="DESKTOP_DEVICE" ? 2 : 4);
...
CGameBase.prototype.gameOver = function(iWinnerIndex, outcome){
    this.setOffTurn();
    ...
    // if(iWinnerNewScore >= GAME_SCORE_WIN){
        ...
        this._bEndGame = true;
        globalThis.gameManager.onGameOver(outcome);
