function CDeckDisplayer (iX,iY,oParentContainer,bHelpAnim){
    ...
    this._init = function(){
        ...
        if(bHelpAnim){
            ...
            _oFinger.scaleX = UI_SIZES_AND_POSITIONS[currentUI].finger().scale;
            _oFinger.scaleY = UI_SIZES_AND_POSITIONS[currentUI].finger().scale;
            _oFinger.x = UI_SIZES_AND_POSITIONS[currentUI].finger().x;
            _oFinger.y = UI_SIZES_AND_POSITIONS[currentUI].finger().y;
    ...
    this.initializeDeck = function(){
        ...
        // Additional 1-9 cards (Each color has two copies)
        for (var i=0;i<4;i++){
            for (var j=0;j<9;j++){
                iTmpSum = j+1;
        ...
        // Special Action Cards: Skip (10), Reverse (11), Draw Two (12), two copies each
        for (var i=0;i<4;i++){
            for (var j=0;j<2;j++){
        ...
        // Wild Cards: Wild Change Color (13), Wild Draw Four (14), four copies each
        for (var i=0;i<4;i++){
    ...
    this.hideHelp = function(){
        _iIDTimeout && clearTimeout(_iIDTimeout);
        _oFingerContainer.visible = false;
