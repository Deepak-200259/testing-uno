function CUnoController(){
    ...
    this._init = function(){
        ...
        var pStartPosButUno = {x: UI_SIZES_AND_POSITIONS[currentUI].game_but_uno().x,y: UI_SIZES_AND_POSITIONS[currentUI].game_but_uno().y};
        _oButUno = new CGfxButton(pStartPosButUno.x,pStartPosButUno.y,oSprite,_oContainer, UI_SIZES_AND_POSITIONS[currentUI].game_but_uno().scale);
        _oButUno.addEventListener(ON_MOUSE_UP,this._onButUno,this);
        
        _oAlertText = new CTLText(_oContainer, 
            UI_SIZES_AND_POSITIONS[currentUI].game_alt_text().left, 
            UI_SIZES_AND_POSITIONS[currentUI].game_alt_text().top, 
            UI_SIZES_AND_POSITIONS[currentUI].game_alt_text().right, 
            UI_SIZES_AND_POSITIONS[currentUI].game_alt_text().bottom, 
            UI_SIZES_AND_POSITIONS[currentUI].game_alt_text().font_size, "center", "#fff", PRIMARY_FONT, 1,
            0, 0,
