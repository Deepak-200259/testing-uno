function CSummaryPanel(oParentContainer){
    ...
    this._init = function(){
        ...
        _oBg.regX = UI_SIZES_AND_POSITIONS[currentUI].summary_msg_box(oBgInfo).regX;
        _oBg.regY = UI_SIZES_AND_POSITIONS[currentUI].summary_msg_box(oBgInfo).regY;
        _oBg.x = UI_SIZES_AND_POSITIONS[currentUI].summary_msg_box(oBgInfo).x;
        _oBg.y = UI_SIZES_AND_POSITIONS[currentUI].summary_msg_box(oBgInfo).y;
        _oBg.scaleX =UI_SIZES_AND_POSITIONS[currentUI].summary_msg_box(oBgInfo).scale;
        _oBg.scaleY =UI_SIZES_AND_POSITIONS[currentUI].summary_msg_box(oBgInfo).scale;
        ...
        _oMsgText = new CTLText(_oGroup, 
                    ...
                    UI_SIZES_AND_POSITIONS[currentUI].game_over_text().font_size, "center", "#fff", PRIMARY_FONT, 1,
                    0, 0,
                    TEXT_SUMMARY,
                    true, true, true,
                    false );
        
        _oWinLoseText = new CTLText(_oGroup, 
            iTextX-iWidth/2, 
            UI_SIZES_AND_POSITIONS[currentUI].summary_win_lose_text(iTextY,iHeight).top,
            iWidth, iHeight, 
            UI_SIZES_AND_POSITIONS[currentUI].summary_win_lose_text(iTextY,iHeight).font_size, "center", "#fff", PRIMARY_FONT, 1,
            0, 0,
            TEXT_SUMMARY,
            true, true, true,
            false );
        ...
        var iY = UI_SIZES_AND_POSITIONS[currentUI].summary_but_home().y;

        oSprite = s_oSpriteLibrary.getSprite("but_home");
        _oButHome = new CGfxButton(UI_SIZES_AND_POSITIONS[currentUI].summary_but_home().x,CANVAS_HEIGHT/2+iY,oSprite,_oGroup);
        _oButHome.addEventListener(ON_MOUSE_DOWN,this._onExit, this);
        
        oSprite = s_oSpriteLibrary.getSprite("but_show");
        _oButShow = new CGfxButton(UI_SIZES_AND_POSITIONS[currentUI].summary_but_show().x,CANVAS_HEIGHT/2+iY,oSprite,_oGroup);
        _oButShow.addEventListener(ON_MOUSE_DOWN,this._onShow, this);
        
        oSprite = s_oSpriteLibrary.getSprite("but_next");
        _oButNext = new CGfxButton(UI_SIZES_AND_POSITIONS[currentUI].summary_but_restart().x,CANVAS_HEIGHT/2+iY,oSprite,_oGroup);
    ...
    this.reset = function(){
        ...
        _oButHome.setX(UI_SIZES_AND_POSITIONS[currentUI].summary_but_home().x);
        _oButShow.setX(UI_SIZES_AND_POSITIONS[currentUI].summary_but_show().x);
        _oButNext.setX(UI_SIZES_AND_POSITIONS[currentUI].summary_but_restart().x);
    ...
    this.endMode = function(szText){    
        _oWinLoseText.refreshText(GAME_OVER_TEXT);    
        if (szText.includes("PLAYER 1")) {
            _oMsgText.refreshText(GAME_WIN_TEXT);
        } else {
            _oMsgText.refreshText(GAME_LOSE_TEXT);
        }
